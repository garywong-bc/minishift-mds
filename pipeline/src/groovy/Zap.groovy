...
routes.items.each {Map route ->
    ...
    OpenShiftHelper._exec(["bash", '-c', "oc process -f openshift/zap.pod.json -l 'zap=${route.metadata.name}' -p 'APP=${appLabel}' -p 'NAME=zap-${route.metadata.name}' -p 'URL=${routeUrl}' --namespace=${route.metadata.namespace} |  oc replace -f - --namespace=${route.metadata.namespace} --force=true"], new StringBuffer(), new StringBuffer())
}
